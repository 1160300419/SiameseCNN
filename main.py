...
parser = argparse.ArgumentParser(description='SiameseCNN')
# learning
...
parser.add_argument('-save-dir', type=str, default='savemodels', help='where to save the snapshot')
...
# option
parser.add_argument('-snapshot', type=str, default='savemodels', help='filename of model snapshot [default: None]')
...
def load_dataset(text_field, label_field, args, **kwargs):
    train_dataset, dev_dataset = mydataset.get_dataset('data', text_field, label_field)
...
train_iter, dev_iter = mydataset.load_data(args)
...
SiaCNN = model.SiameseCNN(args)
# if args.snapshot:
#     print('\nLoading model from {}...\n'.format(args.snapshot))
#     SiaCNN.load_state_dict(torch.load(args.snapshot))

if args.cuda:
    torch.cuda.set_device(args.device)
    SiaCNN = SiaCNN.cuda()
try:
    train.train(train_iter, dev_iter, SiaCNN, args)
