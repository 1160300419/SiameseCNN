...
def load_data(args):
    ...
    text = data.Field(sequential=True, tokenize=tokenizer, stop_words=stop_words)
    '''
    ...
    train, val = data.TabularDataset.splits(
        ...
        validation='dev.csv',
        format='csv',
        fields=[('id', None), ('category',None),('query1', text),('query2', text),('label', label)],
    )
    ...
    train_iter, val_iter = data.BucketIterator.splits(
        (train, val),
        sort_key=lambda x: len(x.query1),
        batch_sizes=(args.batch_size, len(val)), # 训练集设置batch_size,验证集整个集合用于测试
        device=-1   #-1时是运行在cpu上 0是运行在GPU上
    )
    args.vocab_size=len(text.vocab)
    args.pad_id=text.vocab.stoi['<pad>']
